...
class BiLSTM(nn.Module):
    def __init__(self, data):
        ...
        if self.gpu:
            self.drop = self.drop.cuda(0)
            self.droplstm = self.droplstm.cuda(0)
            self.word_embeddings = self.word_embeddings.cuda(0)
            self.biword_embeddings = self.biword_embeddings.cuda(0)
            self.forward_lstm = self.forward_lstm.cuda(0)
            if self.bilstm_flag:
                self.backward_lstm = self.backward_lstm.cuda(0)
            self.hidden2tag = self.hidden2tag.cuda(0)
